...
func (e *storageProviderS3) Start(stop context.Context, await *sync.WaitGroup) error {
	...
	if err := e.Put(ctx, filename, "text/plain", []byte(filedata)); err != nil {
		return fmt.Errorf("missing write access; %s", err)
	}
	if err := e.Delete(ctx, filename); err != nil {
...
func (o *storageProviderS3) Get(ctx context.Context, key string) (io.Reader, error) {
